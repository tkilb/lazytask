...
default urgency coefficients already weight priority heavily. Remaining
Phase 5 sub-features (quick set-priority keys, urgency-based manual
reordering, urgency visibility) still have open design questions and
are not started.
...
- **Phase 5 — Priority logic**: quick set-priority keys, urgency-based
  manual reordering, and urgency visibility remain (priority-based
  coloring and urgency-based default sort chunks are done); each still
  has open design questions needing sign-off before scoping its chunk.
